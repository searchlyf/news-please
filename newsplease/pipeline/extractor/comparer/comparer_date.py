class ComparerDate:
    ...
    def extract(self, item, list_article_candidate):
        ...
        for article_candidate in list_article_candidate:
            if article_candidate.pub_date != None:
                list_pub_date.append(
                    (article_candidate.pub_date, article_candidate.extractor)
                )

        # If there is no value in the list, return None.
        if len(list_pub_date) == 0:
            return None

        # If there are more options than one, return the result from date_extractor.
        list_date_extractor = [x for x in list_pub_date if x[1] == "date_extractor"]
        if len(list_date_extractor) == 0:

            # If there is no date extracted by date_extractor, return the first result of list_pub_date.
            return list_pub_date[0][0]
        else:
